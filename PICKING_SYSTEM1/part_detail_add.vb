...
Public Class part_detail_add
    ...
    Private Sub part_detail_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Finally
            ...
            lb_code_pd.Text = Module1.A_PD 'PD5.lb_code_pd.Text
            lb_code_line.Text = Module1.A_LINE 'PD5.lb_code_line.Text
            Part_No.Text = "Part Selected : " & Module1.A_PAST_NO
            Part_name.Text = "Part Name:" & Module1.A_ITEM_NAME ' PD5.Part_Name.Text
            show_qty.Text = "QTY : " & Module1.A_PAST_QTY
            location.Text = "Location : " & Module1.A_LOCATION
    ...
    Public Sub New()
        InitializeComponent()
        If Api.DownloadImage("http://192.168.161.102/picking_system/uploads/pic/" & Module1.A_PAST_NO & ".jpg") IsNot Nothing Then
            show_img.Image = Api.DownloadImage("http://192.168.161.102/picking_system/uploads/pic/" & Module1.A_PAST_NO & ".jpg")
...
comeback:
        Select Case e.KeyCode
            Case System.Windows.Forms.Keys.Enter
                ...
                req_qty = CDbl(Val(Module1.A_PAST_QTY))
...
exit_keydown:
    ...
    Public Function Ck_dup(ByVal Lis As ListBox, ByVal Str As String)
        ...
        End If
        Dim strCommand3 As String = "SELECT COUNT(id) as c, com_flg  as com_flg , id as i  , scan_qty as qty FROM sup_scan_pick_detail  where item_cd = '" & Module1.A_PAST_NO & "' and scan_lot = '" & order_number & "' and tag_seq = '" & tag_seq & "' and scan_qty >= '" & qty_scan & "' group by com_flg , id , scan_qty"
...
LOOP_INSERT:
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        ...
        If Bluetooth_Connect_MB200i(stInfoSet, pin, pinlen) = True Then
            ...
            Bluetooth_Print_MB200i(stInfoSet, pin, pinlen1, Module1.A_PAST_NO, Module1.A_ITEM_NAME, Module1.A_WI, qty_detail, Module1.A_LINE, Module1.A_USER_ID, now_date_detail, now_time_detail, qrdetailSupply)
        End If
remain_seq_FW:
        If Bluetooth_Connect_MB200i(stInfoSet, pin, pinlen) = True Then
        ...
        Try
            ...
            Dim str_plus As String = "SELECT PICK_QTY , qty FROM sup_work_plan_supply_dev WHERE line_cd  = '" & Module1.M_LINE_CD & "' AND item_cd = '" & Module1.A_PAST_NO & "'AND wi  = '" & Module1.M_WI_STOP_SCAN & "' "
...
loop_check_open_printer:
        If Bluetooth_Connect_MB200i(stInfoSet, pin, pinlen) = True Then
            ...
            Dim qrdetailSupply As String = "SUP " & Module1.A_LINE & " " & Module1.A_WI & " " & itemStrqr & " " & Module1.check_QTY & " " & Module1.A_USER_ID & " " & id_pick_log_supply & " " & date_sup & " " & time_sup
    ...
    Private Sub Bluetooth_Print_MB200i(ByVal stInfoSet As LibDef.BT_BLUETOOTH_TARGET, ByVal pin As StringBuilder, ByVal pinlen As UInt32, ByVal part_number As String, ByVal part_name As String, ByVal wi_code As String, ByVal tag_qty As String, ByVal line_detail As String, ByVal user_detail As String, ByVal date_detail As String, ByVal time_detail As String, ByVal qrdetailSupply As String)
        ...
        Try
            ...
            Else
                ...
                bESC.CopyTo(bBuf, len)
                len = len + bESC.Length
                bBufWork = System.Text.Encoding.GetEncoding(932).GetBytes("V150")
                bBufWork.CopyTo(bBuf, len)
                len = len + bBufWork.Length
...
L_END2:
    ...
    Public Function check_qr_part_in_table()
        ...
        Try
            ...
            ElseIf Len_length = 62 Then 'web post'
                ...
                tag_seq = scan_qty.Text.Substring(59, 3)
            End If
            Dim strCommand As String = "SELECT COUNT(id) as c, com_flg  as com_flg , id as i  , scan_qty as qty FROM check_qr_part  where item_cd = '" & Module1.A_PAST_NO & "' and scan_lot = '" & order_number & "' and tag_seq = '" & tag_seq & "' group by com_flg , id , scan_qty"
            Dim command As SqlCommand = New SqlCommand(strCommand, myConn)
            reader = command.ExecuteReader()
            'MsgBox("strCommand == >" & strCommand)
    ...
    Public Sub inset_check_qr_part()
        ...
        Try
            ...
            set_show_supply()
            strCommand2 = "INSERT INTO check_qr_part (wi,item_cd,scan_qty,scan_lot,tag_typ,tag_readed,scan_emp,term_cd,updated_date,updated_by,tag_seq,S_number , com_flg ,tag_remain_qty  , CREATE_DATE , CREATE_BY , MENU_ID) VALUES ('" & Module1.A_WI & "','" & Module1.A_PAST_NO & "','" & text_tmp.Text & "','" & order_number & "','1','" & scan_qr & "','" & Module1.A_USER_ID & "','" & S_number & "','" & date_now & "','" & Module1.A_USER_ID & "','" & tag_seq & "','" & S_number & "','" & com_flg & "','" & t & "' , '" & date_now & "' , '" & Module1.A_USER_ID & "', '" & Module1.MENU_ID & "')"
            'MsgBox(strCommand2)
            Dim command2 As SqlCommand = New SqlCommand(strCommand2, myConn)
            reader = command2.ExecuteReader()
            reader.Close()
...
re_check:
            ...
            Loop
            reader.Close()
            fa_use_total = (CDbl(Val(scan_qty)) - CDbl(Val(tag_remain_qty))) + Convert.ToInt32(fa_use)
...
L_END2:
    ...
    Public Sub insert_pick_log(ByVal REMAIN_ID As String, ByVal F_wi As String, ByVal used_qty As Double, ByVal create_date As String, ByVal create_by As String, ByVal updated_date As String, ByVal updated_by As String, ByVal scan_lot As String, ByVal updated_seq As String, ByVal F_item_cd As String)
        Try
            ...
            Dim str_insert_log = "INSERT INTO sup_pick_log (REF_ID , WI_NO , USED_QTY , CREATED_DATE , CREATED_BY , UPDATED_DATE , UPDATED_BY , ADD_FLG) VALUES ('" & REMAIN_ID & "','" & F_wi & "' , '" & used_qty & "','" & create_date & "','" & create_by & "' , '" & updated_date & "','" & updated_by & "' , '1')"
